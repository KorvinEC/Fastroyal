...
//    height: 500
    ...
    property int order_list_count: activeOrdersModel.count

    Component {
        id: listViewComponent
        OrderItemNoBtn {
            width: activeOrdersListView.width - 15
            MouseArea {
                anchors.fill: parent
                ...
                onDoubleClicked: {
                    backend.get_order_info(order_id, 'active_orders')
                }
            }
        }
    }//Component
    ...
    Row {
        ...
        CustomButton {
            ...
            onClicked: backend.load_active_orders()
        }
    }

    StackView {
        id: activeOrdersStackView
        anchors.left: parent.left
        anchors.right: parent.right
...
//                anchors.fill: parent
                ListView {
                    ...
                    model: ListModel { id: activeOrdersModel }
    ...
    Text {
        ...
        visible: activeOrdersModel.count == 0 ? true : false

        color: "#ffffff"
        text: qsTr("No active orders")
    ...
    Connections {
        ...
        function addActiveOrder(order_data){
            var data_dict = {};
            ...
            activeOrdersModel.insert(0, data_dict)
        }

        function onActiveOrderSignal(data){
            addActiveOrder(data)
        }

        function onRemoveActiveOrderSignal(order_id){
            console.log(order_id)
            for (var i=0; i<activeOrdersModel.count; ++i){
                if(activeOrdersModel.get(i).order_id == order_id){
                    activeOrdersModel.remove(i)
                }
            }
        }

        function onOrderInfoSignal(order_type_info, order_data){
            if(order_type_info == 'active_orders'){
                var data_dict = {}
                if(order_data.id)                { data_dict['order_id']                 = order_data.id }
                if(order_data.nickname)          { data_dict['order_nickname']           = order_data.nickname}
...
//                data_dict['order_chat_info'] = order_data.order_chat_info
//                data_dict['height'] = activeOrdersItem.height
//                data_dict['width'] = activeOrdersItem.width

            activeOrdersStackView.push(Qt.resolvedUrl('OrderInfoPage.qml'), data_dict)
            }
        }
    }
}
